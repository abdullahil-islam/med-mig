...
class StoreDeliverySlipReport(models.AbstractModel):
    _name = 'report.custom_sale_order.store_report_deliveryslip'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for doc in docs:
            ...
            if sale_order:
                master_product[str(doc.id)] = sale_order.order_line.filtered(lambda item: item.is_master_product).mapped(
                    'product_id').ids
...
class StoreCustomerDeliverySlipReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for doc in docs:
            ...
            sale_order = doc.sale_id
            if sale_order:
                master_product[str(doc.id)] = sale_order.order_line.filtered(
                    lambda item: item.is_master_product).mapped(
                    'product_id').ids
