...
class UserSession(models.Model):
	...
	def save_session(self, uid=False, state=False):
		uid = uid or request.uid
		user = request.env['res.users'].sudo().browse(uid)
		if user._is_public() or uid == 1:
			return False
		...
		browser_info = self.get_browser_name(user_agent.string)
	...
	def get_browser_name(self, user_agent):
		"""Extracts the correct browser name from User-Agent"""
		if "OPR" in user_agent or "Opera" in user_agent or "OPX" in user_agent:
			return "opera"
