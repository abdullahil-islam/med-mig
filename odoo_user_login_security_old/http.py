...
class OpenERPSession(http.Session):
...
class Root(http.Application):
	@func.lazy_property
	def session_store(self):
		# Setup http sessions
		path = config.session_dir
		_logger.debug('HTTP sessions stored in: %s', path)
		return http.FilesystemSessionStore(path, session_class=OpenERPSession, renew_missing=True)
	...
	def get_response(self, httprequest, result, explicit_session):
		if request.db and request.registry.get('session.session') and request.params.get('login_success'):
			...
			if request.uid not in known_users and session:
